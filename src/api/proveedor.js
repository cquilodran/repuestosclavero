...
export async function getListaProveedores(page = 1, limit = 10) {
...
} export async function getListaProveedoresActivos(page = 1, limit = 10) {
  const token = getAccessTokenApi()
  const url = `${basePath}/${apiVersion}/lista-proveedores-activos?page=${page}&limit=${limit}`
  const params = {
    method: "GET",
    headers: {
      "content-Type": "application/json",
