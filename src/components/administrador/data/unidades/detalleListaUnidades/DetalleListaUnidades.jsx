...
const DetalleListaUnidades = (props) => {
  const { state: { docs }, dispatch } = useContext(ContextUnidades)
  ...
  const activarDesactivar = async (id, estado) => {
    const respuesta = await putActDesUnidadesApi(id, !estado)
  ...
  const heads = ["Nombre", "Nombre largo", "Acciones"]
  return (
    <div>
...
function EditarRegistro(props) {
  ...
  const onSubmit = values => {
    setLoading(true)
    editarUnidadesApi(informacion._id, values)
      .then(respuesta => {
        setLoading(false)
