...
import { getListaModeloVehiculoApi, postCrearModeloVehiculoApi } from '../../../../api/modeloVehiculo'
import { ContextModeloVehiculo } from '../../../../context/contextModeloVehiculo'
import { withRouter } from 'react-router-dom'
import queryString from 'query-string'
import Pagination from "react-js-pagination";
import ListaModeloVehiculo from './listaModeloVehiculo'
import FormularioBusquedaModeloVehiculo from './formularioBusqueda'


const ModeloVehiculo = (props) => {
  const { state: { docs, busqueda, actualizando, limit, total }, dispatch } = useContext(ContextModeloVehiculo)
  ...
  function actualizarLista(page) {
    getListaModeloVehiculoApi(page)
      .then(lista => {
        if (lista.ok === false) {
          setModalShow(true)
        } else {
  ...
  if (busqueda) {
    return (
      <div>
        <h2>Buscando modelos de vehiculos</h2>
        <Row>
          <Col md={10}>
        ...
        <hr />
        {
          docs.length > 0 ?
            <>
              <Row>
                <Col>
                  <ListaModeloVehiculo paginaActual={page} />
                </Col>
              </Row>
              {
                total > 10 &&
  ...
  return (
    <div>
      <h2>Lista modelos de vehículos</h2>
      {
        actualizando ?
          <>
...
function CrearRegistro(props) {
  ...
  const onSubmit = values => {
    setLoading(true)
    postCrearModeloVehiculoApi(values)
      .then(respuesta => {
        setLoading(false)
...
export default React.memo(withRouter(ModeloVehiculo))  