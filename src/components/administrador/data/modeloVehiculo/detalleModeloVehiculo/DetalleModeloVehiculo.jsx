...
import { editarModeloVehiculoApi, putActDesModeloVehiculoApi, getListaModeloVehiculoApi } from '../../../../../api/modeloVehiculo'





const DetalleModeloVehiculo = (props) => {
  const { state: { docs }, dispatch } = useContext(ContextModeloVehiculo)
  ...
  const activarDesactivar = async (id, estado) => {
    const respuesta = await putActDesModeloVehiculoApi(id, !estado)
...
function EditarRegistro(props) {
  ...
  const onSubmit = values => {
    setLoading(true)
    editarModeloVehiculoApi(informacion._id, values)
      .then(respuesta => {
        setLoading(false)
  ...
  return (
    ...
    >
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">
          Editando Modelo Vehiculo:  <strong>{informacion.nombre}</strong>
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
        <Form
...
export default React.memo(DetalleModeloVehiculo)
