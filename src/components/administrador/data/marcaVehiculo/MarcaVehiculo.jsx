...
import { postCrearMarcaVehiculoApi, getListaMarcaVehiculoApi } from '../../../../api/marcaVehiculo'
import { ContextMarcaVehiculo } from '../../../../context/contextMarcaVehiculo'
import { withRouter } from 'react-router-dom'
import queryString from 'query-string'
import Pagination from "react-js-pagination";
import ListaMarcaVehiculo from './listaMarcaVehiculo'
import FormularioBusquedaMarcaVehiculo from './formularioBusqueda'


const MarcaVehiculo = (props) => {
  const { state: { docs, busqueda, actualizando, limit, total }, dispatch } = useContext(ContextMarcaVehiculo)
  ...
  if (busqueda) {
    return (
      <div>
        <h2>Buscando marcas de vehiculos</h2>
        <Row>
          <Col md={10}>
            <FormularioBusquedaMarcaVehiculo paginaActual={page} />
          </Col>
          <Col md={2}>
            <OverlayTrigger
              overlay={
        ...
        <hr />
        {
          docs.length > 0 ?
            <>
              <Row>
                <Col>
                  <ListaMarcaVehiculo paginaActual={page} />
                </Col>
              </Row>
              {
                total > 10 ?
  ...
  return (
    <div>
      <h2>Lista marcas de vehículos</h2>
      {
        actualizando ?
          <>
...
function CrearRegistro(props) {
  ...
  const onSubmit = values => {
    setLoading(true)
    postCrearMarcaVehiculoApi(values)
      .then(respuesta => {
        setLoading(false)
